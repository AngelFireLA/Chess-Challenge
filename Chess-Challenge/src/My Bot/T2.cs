...
public class MyBot : IChessBot
{
    ...
    public int Evaluate(Board board) {
        ...
        const int WHITE_SQUARE_BONUS = 1000;
        ulong whiteSquares = 0x55AA55AA55AA55AA;
        ...
        foreach(bool stm in new[] {true, false}) {
            for(var p = PieceType.Pawn; p <= PieceType.King; p++) {
                ...
                while(mask != 0) {
                    ...
                    if(BitboardHelper.SquareIsSet(whiteSquares, ind)) {
                        mg += WHITE_SQUARE_BONUS;
                        eg += WHITE_SQUARE_BONUS;
                    }
                }
    ...
    {   
        ...
        if(!board.IsWhiteToMove) return bestmoveRoot;
        // https://www.chessprogramming.org/Iterative_Deepening
        for(int depth = 1; depth <= 50; depth++) {
            int score = Search(board, timer, -30000, 30000, depth, 0);
                       // Out of time
            if(timer.MillisecondsElapsedThisTurn >= timer.MillisecondsRemaining / 30){
                Console.Out.WriteLine("Achieved  depth of " + depth); 
                break;
            }
        }
